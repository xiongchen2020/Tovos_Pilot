...
import static com.tovos.uav.sample.route.WayPointActivity.aircraftName;
            ...
            for (int i = 0; i < actions.size(); i++) {
                if ("0".equals(actions.get(i).getType())) {//悬停点
                  ...
                  //  waypoint.gimbalPitch = 30;
                    int picth = Math.round(Float.valueOf(actions.get(i).getParam()));
                    if (Model.MAVIC_2_ENTERPRISE_ADVANCED.getDisplayName().equals(aircraftName)){
                       if (Math.round(Float.valueOf(actions.get(i).getParam()))>22){
                           picth = 22;
                       }
                    }
                    if (!isFirstGimbal) {
                        isFirstGimbal = true;
                        waypoint.gimbalPitch = picth;
                    } else {
                        ss = new WaypointAction(WaypointActionType.GIMBAL_PITCH, picth);
                        waypoint.addAction(ss);
                    }
                }
